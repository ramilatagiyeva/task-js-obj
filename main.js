...
function usersId(usersArray) {
    const userMap = new Map();
    usersArray.forEach(user => user.id && userMap.set(user.id, user));
    return userMap;
}
function userRoleId(userMap, id) {
    const user = userMap.get(id);
    console.log(user ? `User with ID ${id} has the role: ${user.role}` : `No user found with ID ${id}`);
}
const userMap1 = usersId(users);
userRoleId(userMap1, 3); 
userRoleId(userMap1, 5); 
...
function deleteUserId(userMap, id) {
    if (userMap.has(id)) {
        userMap.delete(id);
        console.log(`User with ID ${id} has been deleted.`);
    } else {
        console.log(`User with ID ${id} does not exist in the map.`);
    }
}
const userMap2 = usersId(users);
userRoleId(userMap2, 3);
deleteUserId(userMap2, 3);
userRoleId(userMap2, 3);
deleteUserId(userMap2, 3); 
...
const userMap3 = usersId(users);
userRoleId(userMap3, 3); 
deleteUserId(userMap3, 3);
userRoleId(userMap3, 3); 
deleteUserId(userMap3, 3); 
const userObject = mapObject(userMap);
console.log("Object format:", userObject);
const userArray = objectArray(userObject);
console.log("Array format:", userArray);
